...
class OrderProductUserController extends AdminController{

     
    protected $title = 'Order Product User';
 

     protected function grid(){

        $grid = new Grid(new OrderUser());

        $grid->model()->orderBy('id', 'DESC');

       

        $grid->column('userIdsss.fname', __('First Name'));
        $grid->column('userIdsss.lname', __('Last Name'));
        
        $grid->column('title', __('Product Name')); 
        $grid->column('quantity', __('Product Quantity'));

        $grid->column('fixprice', __('Product Orignal price'));
...

    {
        ...
        $show = new Show(OrderUser::findOrFail($id));
